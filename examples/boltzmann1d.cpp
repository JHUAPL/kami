...
BoltzmannWealthModel::BoltzmannWealthModel(unsigned int numberAgents, unsigned int width) {
    ...
    world = new kami::MultiGrid1D(width, true);
    ...
    for (unsigned int i = 0; i < numberAgents; i++) {
        ...
        world->addAgent(newAgent->getAgentID(), static_cast<int>(distWidth(rng)));
    }
}
